...
describe("HybridTextChunker", () => {
  ...
  it("adds deterministic overlap between adjacent chunks", () => {
    const text = [words(8, "a"), words(8, "b"), words(8, "c")].join("\n\n");
    const chunker = new HybridTextChunker({
      ...DEFAULT_CHUNKING_CONFIG,
    ...
    expect(chunks[1]?.content.startsWith("a5 a6 a7")).toBe(true);
    expect(chunks[2]?.content.startsWith("b5 b6 b7")).toBe(true);
  });
