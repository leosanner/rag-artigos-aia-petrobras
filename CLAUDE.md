...
- **Milestone features follow the spec-first workflow.** For any feature listed in [.specs/project/ROADMAP.md](.specs/project/ROADMAP.md) (M1 ingestion, M2 RAG, M3 XAI/observability, M4 agents):
  ...
  4. **Review** — when the code is ready, delegate an independent review to Codex via `codex:rescue`, passing the git diff + the `F-NN` spec file as context. If the user prefers a different reviewer, ask before proceeding.
     - **Extremely important:** every review must use a brand-new reviewer agent/thread. Never reuse the implementation agent or a reviewer that has the implementation conversation in context. The reviewer should receive only the relevant feature markdown contract/detail docs and the git diff needed for review, so it can inspect without implementation bias.
     - Do not mark a review-dependent checklist/status as reviewed or done based only on the implementer's own confidence. Mark it only after the fresh independent reviewer confirms it, or after the user explicitly waives that review requirement.
...
Definitive project name; concrete deployment value for `RAG_GENERATION_MODEL`; agents framework final choice (deferred to M4). F-01 resolves the document-ingestion choices for PDF extraction (`unpdf`), text refinement (deterministic), trigger (`/ingestion` + `POST /api/ingestion/sync`), and background processing (Inngest). Base RAG planning choices now live across `F-02`, `F-03`, and the reordered `/query` contracts `F-04` through `F-08`, covering hybrid chunking, `text-embedding-3-large`, manual async indexing, global RAG, query controls, traceability, reranked retrieval, conversational query, and focused RAG. See [.specs/project/STATE.md](.specs/project/STATE.md) §Todos.
